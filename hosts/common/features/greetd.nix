...
let
  ...
  sway-cmd =
    cmd:
    "${pkgs.dbus}/bin/dbus-run-session ${lib.getExe pkgs.sway} --unsupported-gpu --config ${pkgs.writeText "sway.config" ''
      input "type:keyboard" {
          xkb_layout be,us
          xkb_numlock enabled
      }
      ...
      ${lib.concatMapStringsSep "\n" (m: "output ${m.name} disable") (
        builtins.filter (m: m.primary == false) config.monitors
      )}
